...
Avo.configure do |config|
  ...
  config.authenticate_with do
    # не залогинен → на homepage (или login, если хочешь)
    if current_user.nil?
      redirect_to main_app.root_path, alert: "Нужно войти"
      # залогинен, но не админ → на обычный дашборд
    elsif !current_user.admin?
      redirect_to main_app.dashboard_index_path, alert: "Доступ запрещён"
    end
  end
